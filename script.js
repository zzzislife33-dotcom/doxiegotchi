...
// Add xp to the initial pet object
let pet = { happy: 5, hunger: 5, clean: 5, lv: 1, xp: 0, isDead: false };
...
function resetGame() { 
    // 1. Clear everything
    localStorage.clear(); 
    pet = { happy: 5, hunger: 5, clean: 5, lv: 1, xp: 0, isDead: false };
    
    // 2. Reset the UI visibility immediately
...
function pressB() {
    ...
    if(selectedIdx === 0) { 
        if(pet.hunger < 5) { 
            pet.hunger++; 
            canAwardXP = true; 
            reactionSrc = "assets/icon_hungry.png"; 
        }
    }
        ...
        if(pet.happy < 5) { 
            ...
            canAwardXP = true; 
            reactionSrc = "assets/icon_happy.png"; 
        }
    }
        ...
        if(pet.clean < 5) {
            pet.clean = 5; 
            canAwardXP = true; 
            reactionSrc = "assets/icon_clean.png";
        }
    }
    ...
    if(canAwardXP) { 
        ...
        if (pet.xp >= 100) {
            pet.lv += 1;
            pet.xp = 0;
            // REMOVED: Stat reset logic. 
            // Stats now stay exactly where they were before the level up.
            playBeep(1500, 'square', 0.3); 
        }
...
function render() {
    ...
    // Swapped order: hunger logic first to match HTML display
    const stats = [
    ...
    stats.forEach(s => {
        ...
        // Math.ceil rounds 4.5 up to 5, so the user sees 5 dots
...
setInterval(() => {
    ...
    // 2. Logic for Inner Room Background
    // Day: 7 AM to 6:59 PM (19:00) | Night: 7 PM to 6:59 AM
    const isDayTime = (hours >= 7 && hours < 19);
    
    const screen = document.getElementById('screen');
    
    // This ensures it uses your specific filenames
    if (isDayTime) {
        screen.style.backgroundImage = "url('assets/bg_day.png')";
    } else {
        screen.style.backgroundImage = "url('assets/bg_night.png')";
    }
